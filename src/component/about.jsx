    ...
    <div class="w-screen flex h-screen relative">
      ...
      <div className='flex flex-col justify-center items-center z-10 pt-20'>
        <div className='w-[90%] max-w-screen-xl'>
          <h3 className='text-[28px] text-[#FDCBCB] text-center font-sansation'>Global Gate Services is a place where businesses come alive with the pulse of AI-driven insights, where every click sparks new possibilities, and where learning is as delightful as it is enlightening. That's the vision behind Globalgate – a vision fueled by passion, powered by technology, and dedicated to empowering businesses like yours to thrive in the global marketplace</h3>
        </div>
        <div>
          <h1 className='text-[60px] font-bold text-white pt-8 font-sansation'>Ai-Powered Market Insights</h1>
        </div>
        <div className='flex w-[90%] flex-wrap justify-center mt-8'>
          {InsightsData.map((item, index) => (
            <div key={index} className='w-[32%]'>
              <div className='flex flex-row px-2 py-4'>
                <div className='mr-5 flex flex-col'>
                  <div className='w-[50px] h-[50px] bg-[#8FD5FC] rounded-[10px] border-[#DED72B] border-2 text-center text-black text-[32px] font-semibold font-sansation'>
                    {index + 1}
                  </div>
                </div>
                <div className='flex-col flex-grow'>
                  <p className='text-[32px] text-[#FDCBCB] font-bold font-sansation'>{item.title}</p>
                  <p className='text-[18px] text-white font-medium font-sansation'>{item.subTitle}</p>
                </div>
              </div>
            </div>
          ))}
